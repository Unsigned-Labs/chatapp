...
export class NIP59GiftWrapper {
	wrapEvent(message: string, recipientPubkey: string, senderPrivateKey: Uint8Array) {
	...
	unwrapEvent(wrap: Event, recipientPrivateKey: Uint8Array) {
		const nip44ConversationKey = (privateKey: Uint8Array, publicKey: string) =>
			nip44.v2.utils.getConversationKey(bytesToHex(privateKey), publicKey);
